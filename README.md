...
There are a number of ways to manage AWS Lambda functions and layers already,
but their generality and all-encompassing approaches don't integrate well with
certain workflows. This project aims to solve AWS Lambda function and layer
version deployment and alias management for Python runtimes where the actual
infrastructure is managed separately, mostly likely by CloudFormation or 
Terraform.

The goal of this project is to facilitate the deployment and updating of AWS
Lambda Python functions and layers for all of the version-specific
configurations, e.g. code bundles, environment variables, memory size, and
timeout lengths. The expectation is that functions are created by other means
and then configuration for versions is managed with the lambda-deployer through
an interactive or scripted shell of commands.
...
will be managed by lambda-deployer. A bucket must be specified to indicate
where the zipped code bundles will be uploaded prior to them being applied to
the target(s).
...
`lambda.yaml` file is located that can be determined to be a python package
directory as identified by an `__init__.py` file within the folder. It will
also look for a `lambda_function.py` file at the same level as the 
`lambda.yaml` file to serve as the entrypoint. These will be included in the
uploaded and deployed code bundle when a `push` or a `deploy` command is
executed. These default settings can all be configured along with many more as
will be outlined below.

To deploy this example project, install the lambda-deployer python library and
start the shell with the command `lambda-deployer` in your terminal of choice
in the directory where the `lambda.yaml` file resides. Then run the `push` 
command to create and upload the bundled source code and then publish a new
version of the `foo-function` lambda function with the uploaded results.

# Shell commands

The lambda-deployer command starts an interactive shell that contains various
...
is described later in this document. A brief description of the available
commands are:

- alias: Creates and assigns an alias to the specified version of the
    selected or specified lambda function, or reassigns an existing alias to a
    different version.
- bundle: Installs dependencies and copies includes into a zipped file
    that is structured correctly to be deployed to the lambda function/layer
    target.
- configs: Displays the configs loaded from the lambda.yaml file and
    fully populated with defaults and dynamic values.
- deploy: Uploads the bundled contents to the upload S3 bucket and then
    publishes a new version of each of the lambda targets with that new bundle
    and any modified settings between the current configuration and that
    target's existing configuration.
- exit: Exits the shell and returns to the calling terminal.
- help: Displays help information on the commands available within the
    shell. Additional help on each command can be found using the --help flag on
    the command in question.
- list: Lists versions of the specified lambda targets with info about
    each version.
- prune: Removes old function and/or layer versions for the selected
    targets.
- push: Combined single command for bundling and deploying the selected
    targets.
- reload: Reloads the lambda.yaml configuration file from disk.
- select: Allows for selecting subsets of the targets within the loaded
    configuration. The subsets are fuzzy-matched unless the --exact flag is
    used.
- shell: Special command to use in run command groups/macros to start
    interactive command mode for the terminal. Useful when in scenarios where
    you wish to prefix an interactive session with commonly executed commands.
    For example, if you want to select certain targets with the select command
    as part of starting the shell, you could create a run command group/macro
    in your lambda.yaml that executes the select command and then executes the
    shell command. This would updated the selection and then with the shell
    command, start the shell in interactive mode. Without specifying the shell
    command here, the run command group/macro would just set a selection and
    then exit.
- status: Shows the current status information for each of the selected
    lambda targets.
    
More detail on any of these commands can be found from within the shell by
executing them with the `--help` flag.

The lambda-deployer application also supports non-interactive batch command
...
source and should be read-accessible by the profile invoking lambda-deployer.
...
prevent changes being applied by lambda-deployer.
...
*-1*. After those three commands are executed lambda-deployer will exit the
